...
f1 = fopen('Result data\\exp9\\SimRes_exp9_1.csv');
f2 = fopen('Result data\\exp9\\SimRes_exp9_2.csv');
f3 = fopen('Result data\\exp9\\SimRes_exp9_3.csv');
f4 = fopen('Result data\\exp9\\AnaRes_exp9_1.csv');
f5 = fopen('Result data\\exp9\\AnaRes_exp9_2.csv');
f6 = fopen('Result data\\exp9\\AnaRes_exp9_3.csv');
...
plot(x,y1_D,'k', x,y2_D,'r', x,y3_D,'b',...
     x,y4_D,'k+', x,y5_D,'r+', x,y6_D,'b+',...
     x,y1_I,'k', x,y2_I,'r', x,y3_I,'b',...
     x,y4_I,'ko', x,y5_I,'ro', x,y6_I,'bo');
axis([0, 50, 0.0, 1.0]);
xlabel('t(time)'); % x軸註解
ylabel('The fraction of I and D'); % y軸註解
title('Different death rate of SEIRD model'); % 圖形標題
legend('Delta = 0.05, Ex = 0.01',...
       'Delta = 0.04 , Ex = 0.06',...
       'Delta = 0.4 , Ex = 0.1'); % 圖形註解